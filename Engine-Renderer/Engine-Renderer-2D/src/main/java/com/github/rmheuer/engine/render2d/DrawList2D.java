...
public class DrawList2D {
    ...
    public void drawImage(float x, float y, float w, float h, Image img, Vector4f tint, float u0, float v0, float u1, float v1) {
        ...
        int mark = vertices.size();
        vertex(min.x, min.y, MathUtils.map(min.x, x, x + w, u0, u1), MathUtils.map(min.y, y, y + h, v0, v1), tint, img);
        vertex(max.x, min.y, MathUtils.map(max.x, x, x + w, u0, u1), MathUtils.map(min.y, y, y + h, v0, v1), tint, img);
        vertex(max.x, max.y, MathUtils.map(max.x, x, x + w, u0, u1), MathUtils.map(max.y, y, y + h, v0, v1), tint, img);
        vertex(min.x, max.y, MathUtils.map(min.x, x, x + w, u0, u1), MathUtils.map(max.y, y, y + h, v0, v1), tint, img);
        indices.add(mark);
        indices.add(mark + 1);
        indices.add(mark + 2);
        indices.add(mark);
